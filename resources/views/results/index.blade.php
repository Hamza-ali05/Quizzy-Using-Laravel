  ...
  <table class="table table-bordered mt-3">
    ...
    <tbody>
      ...
      @if($user->role === 'admin')
        ...
        @forelse($attempts->filter(fn($a) => optional($a->quiz)->creator_id === $user->id) as $attempt)
          <tr>
            <td>{{ optional($attempt->member)->name }}</td>
            <td>{{ optional($attempt->quiz)->title }}</td>
            <td>{{ $attempt->marks ?? 0 }}</td>
      ...
      @elseif($user->role === 'student')
        ...
        @forelse($attempts->filter(fn($a) => $a->member_id === $user->id) as $attempt)
          <tr>
            <td>{{ optional($attempt->member)->name }}</td>
            <td>{{ optional($attempt->quiz)->title }}</td>
            <td>{{ $attempt->marks ?? 0 }}</td>
      ...
      @else
        <tr>
          <td colspan="4" class="text-center text-muted">You do not have permission to view results.</td>
        </tr>
      @endif
    </tbody>
  </table>
